...
public class FirmCodeProvider extends BatchDisplayTextProviderAdaptor {
	...
	public List<ValuePair<?>> getLookupList(Object val, Map metaMap, FieldMeta fieldMeta) {
		...
		example.createCriteria().andIn("firmState", Arrays.asList(FirmState.ENABLED.getValue(), FirmState.DISABLED.getValue()));
		List<Firm> list = firmService.selectByExample(example);
		list.forEach(firm -> {
	...
	/**
	 * 返回主DTO和关联DTO需要转义的字段名 Map中key为主DTO在页面(datagrid)渲染时需要的字段名， value为关联DTO中对应的字段名
	 * 
	 * @return
	 */
	@Override
	...
	/**
	 * 主DTO与关联DTO的关联(java bean)属性(外键) 先从field属性取，没取到再取_fkField属性 子类可自行实现
	 * 
	 * @return
	 */
	@Override
